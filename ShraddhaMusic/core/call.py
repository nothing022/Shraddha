...
from pytgcalls.types import Update,AudioQuality,VideoQuality,MediaStream,ChatUpdate
from pytgcalls.types.stream import StreamAudioEnded
import config
from ShraddhaMusic import LOGGER, YouTube, app
...
from ShraddhaMusic.utils.formatters import check_duration, seconds_to_min, speed_converter
from ShraddhaMusic.utils.inline.play import stream_markup
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if "live_" in queued:
                ...
                file_path, direct, error = await YouTube.download(
                        videoid,
                        mystic,
                        videoid=True,
                        video=True if str(streamtype) == "video" else False,
                ...
                if not file_path:
                  return await mystic.edit_text(f"""{_["call_6"]} \n\n Reason : {error}""",disable_web_page_preview=True)
                if video:
                    stream = MediaStream(
                        file_path,
                        audio_parameters=AudioQuality.STUDIO,
                    ...
                    stream = MediaStream(file_path, audio_parameters=AudioQuality.STUDIO)
                try:
                    await client.play(chat_id, stream)
                except:
                    return await app.send_message(
