...
from ShraddhaMusic.misc import SUDOERS, db
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Shraddha.pause_stream(chat_id)
        ...
        await Shraddha.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention), reply_markup=close_markup(_)
        )
    elif command in ["Seeknext","Seekback"]:
        seeker = "e" if (command == "Seeknext") else "c"
        await CallbackQuery.answer(_["admin_41" if (command == "Seeknext") else "admin_42"], show_alert=True)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await Shraddha.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Shraddha.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            file_path, direct, error = await YouTube.download(videoid,mystic,videoid=True,video=status)
            if not file_path:
            ...
            try:
                ...
                image = None
            try:
                await Shraddha.skip_stream(chat_id, file_path, video=status, image=image)
            except:
            ...
            try:
                await Shraddha.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Shraddha.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
