...
from ShraddhaMusic.utils.database import get_assistant, get_authuser_names, get_cmode
...
async def restartbot(client, message: Message, _):
    ...
    try:
        db[message.chat.id] = []
        await Shraddha.stop_stream_force(message.chat.id)
    except:
        pass
    ...
    if chat_id:
        ...
        try:
            db[chat_id] = []
            await Shraddha.stop_stream_force(chat_id)
        except:
            pass
...
async def close_menu(_, CallbackQuery):
    try:
        ...
        mes = await CallbackQuery.message.reply_text(f"Cʟᴏsᴇᴅ ʙʏ : {CallbackQuery.from_user.mention}")
